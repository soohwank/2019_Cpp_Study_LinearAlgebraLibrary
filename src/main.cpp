...
public:
	...
	// dotProduct
	Vector3d dotProduct(const Vector3d &v) const
	{
		Vector3d result;
		...
		cout << result.data[0] + result.data[1] + result.data[2] << endl;
	...
	Vector3d crossProduct(const Vector3d &v) const
	{
		...
		cout << result.data[0] << 'i' << " + "
			<< result.data[1] << 'j' << " + "
			<< result.data[2] << 'k' << endl;
...
int main()
{
	...
	// dot product
	Vector3d dotProduct = v1.dotProduct(v2);
